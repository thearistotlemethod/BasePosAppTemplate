...
class OfflineAdvice @Inject constructor(
    private val context: Context,
    private val databaseService: DatabaseService,
    private val state: State
) : BaseProcess(context, databaseService, state) {

    fun prepareOfflineAdviceMsg(isoMsg: IMessage): Int {
        val rv = 0
        ...
        if (state.tranData.OrgAmount.length > 0)
            isoMsg.addField("4", state.tranData.OrgAmount.padStart(12, '0'))
        else
        ...
        if (state.tranData.ProcessingCode != 0)
            isoMsg.addField("37", state.tranData.RRN)

        if (state.tranData.AuthCode.length > 0)
